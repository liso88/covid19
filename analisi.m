...
decessi = city.deceduti;
decessi_giornalieri = [0; diff(decessi)];
...
legend('data','fit', 'previsione');
xlabel('Giorni')
ylabel('Decessi totali')
title('Decessi per '+ string(cityName))

%% confronto andamento 3 giorni
figure
hold on
title('Decess, Confronto di 3 giorni per '+ string(cityName))
y2 = y(1:end-2);
y1 = y(1:end-1);
plot(x_,y,'*')
...
figure
hold on
title('Decessi, Trend Previsioni per '+ string(cityName))
previsione2 = f(F2_fitted,[0:timeHorizon]);
previsione1 = f(F1_fitted,[0:timeHorizon]);
previsione = f(F_fitted,[0:timeHorizon]);
...
%f = @(F,x) F(1) ./ (1 +  exp(-F(2).*(x - F(3))));
%f(x) =  F(1).*exp(-((x-F(2))./F(3)).^2)
decessi_giornalieri(decessi_giornalieri<0) = 0;
fg = @(F,x) F(1).*exp(-((x-F(2))./F(3)).^2);
fgiorn = fit(x,decessi_giornalieri,'gauss1');
fgiorn2 = fit(x,decessi_giornalieri,'gauss2');

previsione2 = feval(fgiorn,[0: timeHorizon]);
previsione3 = feval(fgiorn2,[0: timeHorizon]);

plot(x_,decessi_giornalieri,'*k')
hold on
%plot(x_,[0; diff(f(F_fitted,x))],'g');
plot([Date(1) : Date(1)+timeHorizon],[0 diff(previsione)], 'r');
plot([Date(1) : Date(1)+timeHorizon],previsione2, 'g');
...
legend('data','derivata logistica.','gauss1', 'gauss2','gauss2 smooth3');
xlabel('Giorni')
ylabel('Decessi Giornalieri')
title('Variazione decessi giornaliero per '+ string(cityName))